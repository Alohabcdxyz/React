...
import {
  getAllPatientForDoctorNotConfirm,
  sendCancelDetail,
...
class ManageNotConfirmBooking extends Component {
  constructor(props) {
    super(props);
    this.state = {
  ...
  getDataPatient = async () => {
    ...
    let res = await getAllPatientForDoctorNotConfirm({
      doctorId: user.id,
      date: formattedDate,
    });
  ...
  sendRemedy = async (dataFromChild) => {
    ...
    let res = await sendCancelDetail({
    ...
    if (res && res.errCode === 0) {
      ...
      toast.success("Send Note Success");
      await this.getDataPatient();
      this.closeModal();
    } else {
      this.setState({
        ...
        >
          <Row className='manage-booking-container'>
            <Col className='manage-booking-title' span={24}>
              <FormattedMessage id='menu.doctor.manage-not-confirm-booking' />
            </Col>
                      ...
                      return (
                        <tr key={index}>
                          ...
                          <td>
                            ...
                            >
                              <FormattedMessage id='menu.doctor.cancel' />
                            </Button>
                          </td>
                        </tr>
                      );
